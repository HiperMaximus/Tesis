...
class Conv2dZeroBias(nn.Conv2d):
    ...
    def reset_parameters(self) -> None:
        ...
        if self.bias is not None:
            nn.init.zeros_(tensor=self.bias)
...
class PReLUWithBias(nn.Module):
    ...
    def __init__(self, num_channels: int, prelu_init: float = 0.25, prelu_num_parameters: int = 1) -> None:
        ...
        self.bias = nn.Parameter(data=torch.full(size=(1, num_channels, 1, 1), fill_value=0.0))
...
class CustomConvNeXtBlock(nn.Module):
    ...
    def __init__(self, num_channels: int, num_res_blocks: int) -> None:
        ...
        expanded_dim: int = 4 * num_channels  # expansion factor used in ConvNeXt

        # Compute the scaling factor for weight layers in the residual branch.
        # For m=3, the scale becomes L^(-1/(2*3-2)) = L^(-1/4)
        scale: float = num_res_blocks ** (-1 / (2 * m - 2))

        # Define the layers with explicit bias addition
        self.initial_bias = nn.Parameter(data=torch.full(size=(1, num_channels, 1, 1), fill_value=0.0))
        ...
        self.pwconv1.weight.data.mul_(other=scale)  # Scale weights
        ...
        self.dwconv.weight.data.mul_(other=scale)  # Scale weights
        ...
        nn.init.constant_(tensor=self.pwconv2.weight, val=0)

        # Define the activation functions with explicit bias addition.
        self.prelu1 = PReLUWithBias(num_channels=expanded_dim)
        self.prelu2 = PReLUWithBias(num_channels=expanded_dim)

        # Learnable scaling parameter for the output of the residual branch
        self.layer_scale = nn.Parameter(data=torch.ones(1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        residual: torch.Tensor = x  # Store input for the residual connection
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        num_res_blocks: int = num_upsamples * num_blocks_per_scale
        # Regularize initial training
        self.constant_bias = nn.Parameter(data=torch.zeros(self.latent_dim * 4 * 4))
        self.noise_scale = nn.Parameter(data=torch.ones(1) * 0.1)  # Small initial scale
        self.input_linear: Callable[[torch.Tensor], torch.Tensor] = nn.Linear(
            in_features=self.latent_dim,
            out_features=self.base_channels * 4 * 4,
            bias=True,
        )
        ...
        for _ in range(num_upsamples):
            ...
            self.upsample_stages.append(module=nn.Sequential(*blocks))
        ...
        self.to_rgb = nn.Conv2d(in_channels=base_channels, out_channels=img_channels, kernel_size=1, bias=True)
        nn.init.zeros_(tensor=self.to_rgb.weight)
        if self.to_rgb.bias is not None:
            nn.init.zeros_(tensor=self.to_rgb.bias)  # Initialize bias to 0
    ...
    def forward(self, input_noise: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(size=(-1, self.base_channels, 4, 4))
        ...
        for stage in self.upsample_stages:
            ...
            x: torch.Tensor = nn.functional.interpolate(input=x, scale_factor=2, mode="bilinear", align_corners=False)
            x = stage(x)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.linear_1: Callable[[torch.Tensor], torch.Tensor] = nn.Linear(
            in_features=base_channels * 4 * 4,
            out_features=base_channels * 4 * 4,
        )
        self.linear_activation = PReLUWithBias(num_channels=base_channels * 4 * 4)
        # Project down to the desired latent dimension.
        self.linear_2: Callable[[torch.Tensor], torch.Tensor] = nn.Linear(
            in_features=base_channels * 4 * 4,
            out_features=latent_dim,
        )
        self.final_activation: Callable[[torch.Tensor], torch.Tensor] = PReLUWithBias(
            num_channels=latent_dim,
            prelu_init=0.9,
            prelu_num_parameters=latent_dim,
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for stage in self.downsample_stages:
            x = stage(x)
            x = nn.functional.interpolate(input=x, scale_factor=0.5, mode="bilinear", align_corners=False)
...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        num_res_blocks: int = num_downsamples * num_blocks_per_scale
        ...
        for _ in range(num_downsamples):
            ...
            self.downsample_stages.append(module=nn.Sequential(*blocks))
        ...
        feature_dim: int = base_channels * 4 * 4
        # Allow concatenating the projected latent vector
        feature_dim *= 2

        # Linear layers for final classification on the joint feature.
        self.linear_1: Callable[[torch.Tensor], torch.Tensor] = nn.Linear(in_features=feature_dim, out_features=feature_dim)
        self.linear_activation = PReLUWithBias(num_channels=feature_dim, prelu_num_parameters=feature_dim)
        self.linear_2: Callable[[torch.Tensor], torch.Tensor] = nn.Linear(in_features=feature_dim, out_features=1)
        ...
        self.latent_projection: Callable[[torch.Tensor], torch.Tensor] = nn.Linear(
            in_features=latent_dim,
            out_features=feature_dim,
        )
        # Latent activation to apply after the projection
        self.latent_activation: Callable[[torch.Tensor], torch.Tensor] = PReLUWithBias(num_channels=1, prelu_init=0.9)
    ...
    def forward(self, x: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        ...
        f_img: torch.Tensor = x.view(x.size(dim=0), -1)

        # Project latent z.
        f_z: torch.Tensor = self.latent_projection(z)
        f_z = self.latent_activation(f_z)

        # Concatenate along the feature dimension.
        # This yields a feature vector of dimension: feature_dim*2.
        f_joint: torch.Tensor = torch.cat(tensors=(f_img, f_z), dim=1)

        # Process the joint feature through the linear layers.
        out: torch.Tensor = self.linear_1(f_joint)
...
# BiGANLoss
###############################################################################
class BiGANLoss(nn.Module):
    """Module to calculate the full losses for the BiGAN.

    It computes:
      - Generator/Encoder loss: using a relativistic adversarial formulation with roles swapped
        plus a latent regressor loss (enforcing E(G(z)) ≈ z).
      - Discriminator loss: using the discriminator's relativistic loss method (with R1 and R2 gradient penalty).

    The module holds references to the generator, encoder, and discriminator to allow for good compilation.
    """
    ...
    def __init__(
        ...
        latent_regressor_weight: float = 0.1,
    ) -> None:
        """Initialize the nn.Module to do the actual training by passing the Encoder, Generator and Discriminator."""
        super().__init__()
        self.generator: Generator = generator
        self.encoder: Encoder = encoder
        self.discriminator: Discriminator = discriminator
        self.latent_regressor_weight: float = latent_regressor_weight

    def f_least_squares(self, v: torch.Tensor) -> torch.Tensor:
        """Least squares classification loss: (1 - v)**2."""
        return torch.square(input=1 - v).mean()
    ...
    def compute_disc_loss(
        ...
        gamma: float,
        grad_penalty_type: list[Literal["R1", "R2"]] | None = None,
    ) -> torch.Tensor:
        ...
        Args:
            ...
            grad_penalty_type (Literal["R1","R2"]): If "R1", apply R1 penalty (real data); if "R2", apply R2 penalty (fake data).
        ...
        if grad_penalty_type is None:
            grad_penalty_type = ["R1", "R2"]
        pred_real: torch.Tensor = self.discriminator(real_images, real_latents)  # D(x, E(x))
        pred_fake: torch.Tensor = self.discriminator(fake_images, fake_latents)  # D(G(z), z)

        # Compute relativistic difference.
        v: torch.Tensor = pred_fake - pred_real
        adv_loss: torch.Tensor = self.f_least_squares(v=v)

        # ----- Gradient Penalties -----
        # Compute gradient penalty based on flag.
        if "R1" in grad_penalty_type:
            ...
            real_images_gp: torch.Tensor = real_images.clone().detach().requires_grad_(mode=True)
            # Compute discriminator outputs for these real inputs.
            pred_real_gp: torch.Tensor = self.discriminator(real_images_gp, real_latents)

            # Prepare gradient outputs (ones tensor of the same shape as pred_real_gp).
            grad_outputs_real: torch.Tensor = torch.ones_like(input=pred_real_gp)
            ...
            gradients_real: torch.Tensor = gradients_real.view(real_images.size(dim=0), -1)

            # Compute the R1 penalty (gamma * mean squared norm of gradients)
            adv_loss += gamma * torch.mean(input=torch.sum(input=gradients_real**2, dim=1))
        if "R2" in grad_penalty_type:
            # R2: Zero-centered gradient penalty on fake data
            # Create a copy of fake_images with gradient tracking enabled.
            fake_images_gp: torch.Tensor = fake_images.clone().detach().requires_grad_(mode=True)

            # Compute discriminator outputs for fake images.
            pred_fake_gp: torch.Tensor = self.discriminator(fake_images_gp, fake_latents)

            # Prepare gradient outputs (ones tensor of the same shape as pred_fake_gp).
            grad_outputs_fake: torch.Tensor = torch.ones_like(input=pred_fake_gp)

            # Compute gradients of D_fake_gp with respect to fake_images_gp.
            gradients_fake: torch.Tensor = torch.autograd.grad(
            ...
            # Reshape gradients per sample.
            gradients_fake = gradients_fake.view(fake_images.size(dim=0), -1)

            # Compute the R2 penalty (gamma * mean squared norm of gradients)
            adv_loss += gamma * torch.mean(input=torch.sum(input=gradients_fake**2, dim=1))

        # Total Discriminator Loss includes gradient penalties
        return adv_loss

    def calculate_gen_enc_loss(
        self,
        real_images: torch.Tensor,
        fake_latents: torch.Tensor,
        latent_regressor_weight: float = 0.1,
    ) -> torch.Tensor:
        """Calculate the adversarial loss for the generator and encoder, including a latent regressor loss.

        Args:
            real_images (torch.Tensor): Batch of real images.
            fake_latents (torch.Tensor): Latent vectors sampled from p(z), shape (batch_size, latent_dim).
            latent_regressor_weight (float): Weight for the latent regressor loss term.

        Returns:
            torch.Tensor: Scalar loss for the generator and encoder.

        """
        # For real pairs: (x, E(x))
        real_latents: torch.Tensor = self.encoder(real_images)
        pred_real: torch.Tensor = self.discriminator(real_images, real_latents)

        # For fake pairs: (G(z), z)
        fake_images: torch.Tensor = self.generator(fake_latents)
        pred_fake: torch.Tensor = self.discriminator(fake_images, fake_latents)

        # Compute adversarial loss for G/E using the relativistic least-squares loss.
        # We swap the roles: we want to fool the discriminator, so we define:
        # D(G(z), z) - D(x, E(x)) = v
        # and aim to minimize (1 - v_gen_enc)^2.
        # Compute relativistic difference.
        v: torch.Tensor = pred_fake - pred_real
        adv_loss: torch.Tensor = self.f_least_squares(v=v)

        # Latent regressor loss: enforce that E(G(z)) approximates the original z.
        reconstructed_latents: torch.Tensor = self.encoder(fake_images)
        latent_loss: torch.Tensor = nn.functional.mse_loss(input=reconstructed_latents, target=fake_latents)

        # Total loss: adversarial plus latent regressor.
        total_loss: torch.Tensor = adv_loss + latent_regressor_weight * latent_loss
        return total_loss
