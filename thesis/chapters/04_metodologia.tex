...
subconjunto de máscaras suplementarias con tres categorías: tumor, estroma y
necrosis. Estas máscaras no cubren de manera densa todas las láminas.
...
Otsu~\cite{otsu_threshold_1979} y se proyectó una cuadrícula de
\(256\times256\) píxeles a la resolución
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/dataset_atlas_pipeline}}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/wsi_patch_storage}}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/dataset_task_views}}
...
espacio de hematoxilina, eosina y residuo, obtenido mediante deconvolución de
color~\cite{ruifrok_color_2001}, junto con ruido gaussiano, mientras que el
...
\begin{figure}[htbp]
    \centering
    \resizebox{0.94\textwidth}{!}{\input{figures/architecture/residual_block}}
    \caption[Estructura del bloque residual empleado.]{Estructura de un bloque residual. La suma combina la rama
...
bloques residuales recorrieron anchos de 32, 48, 64 y 96 canales. Cada rama
    principal combinó convoluciones de \(5\times5\), normalización por grupos
    ~\cite{wu_groupnorm_2018} y la
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/normal_vae}}
...
\begin{figure}[htbp]
    \centering
    \resizebox{0.96\textwidth}{!}{\input{figures/architecture/field_types}}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/equivariant_convolution}}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/kernel_parameter_sharing}}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/so2_vae}}
...
Los modelos se optimizaron con AdamW~\cite{loshchilov_adamw_2019}, coeficientes
\((0,9;0,999)\),
...
\begin{figure}[p]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/mil_local_global_detail}}
...
La notación de consultas, claves, valores y escalamiento por
\(\sqrt{d_h}\) sigue la atención de producto punto de Vaswani et al.
~\cite{vaswani_attention_2017}; las normalizaciones LN y las unidades SwiGLU
son las definidas en el marco de referencia
~\cite{ba_layernorm_2016,shazeer_glu_2020}. Las ecuaciones siguientes precisan
la conectividad particular implementada para las WSI.
...
\begin{figure}[p]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/local_attention_computation}}
...
miniatura. En el flujo de datos empleado, el rojo se interpretó como tumor, el
verde como estroma y el azul como necrosis. En inferencia, el mismo
...
\begin{figure}[htbp]
    ...
    \caption[Ejemplo de segmentación de WSI a resolución de parche.]{Ejemplo real de la segmentación de WSI a resolución de parche para
    ...
    pero no de necrosis. Fuente: UBC-OCEAN, licencia CC BY-NC-ND 4.0
    ~\cite{ubc_ocean_2023}; miniatura y máscara reducidas, y superposición de
    cuadrícula elaborada para esta tesis.}
...
activación GELU~\cite{hendrycks_gelu_2016}, agrupación promedio adaptativa y
una capa lineal
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/architecture/downstream_heads}}
...
Cada presupuesto se optimizó por un máximo de 30 épocas con AdamW
~\cite{loshchilov_adamw_2019} y tasa máxima
...
Como sondeo de contenido, la imagen RGB original se redujo de
\(256\times256\) a \(32\times32\) mediante interpolación bilineal con
antialiasing. En 20 de los 25 parches se ajustó una transformación afín
\(\mathbb R^{16}\rightarrow\mathbb R^3\) entre el vector latente de cada
posición y su color, con estandarización de las 16 variables y regresión
\emph{ridge} de penalización \(10^{-3}\) sin penalizar el intercepto. Los cinco
parches restantes, fijados antes del ajuste, se usaron para calcular
\(R^2=1-\mathrm{SSE}/\mathrm{SST}\); en \(\mathrm{SST}\), la referencia fue la
media RGB de los 20 parches de ajuste. Este sondeo mide información cromática
recuperable mediante una lectura lineal local, no calidad diagnóstica.
...
canonizar el latente. Para definir las razones reportadas, sea
\(\mathbf z_0=E(\mathbf x)\), \(\mathbf y_0=D(\mathbf z_0)\) y
\(\mathbf z_\theta=E(R_\theta\mathbf x)\). Si

\begin{equation}
    \lVert\mathbf a\rVert_{\Omega,M}
    =\left[\frac{1}{|\Omega||M|C}
      \sum_{\theta\in\Omega}\sum_{u\in M}\sum_{c=1}^{C}
      a_{\theta c u}^{2}\right]^{1/2},
    \label{eq:norma-rms-orbita}
\end{equation}

denota la RMS sobre los ángulos no nulos \(\Omega\), los \(C=3\) canales RGB y
una máscara espacial central \(M\), se calcularon

\begin{equation}
\begin{aligned}
    r_{\mathrm{act}}
      &=\frac{\lVert D(R_\theta\mathbf z_0)-R_\theta\mathbf y_0
          \rVert_{\Omega,M}}
        {\lVert\mathbf y_0-R_\theta\mathbf y_0\rVert_{\Omega,M}+\epsilon},\\
    r_{\mathrm{can}}
      &=\frac{\lVert D(R_{-\theta}\mathbf z_\theta)-\mathbf y_0
          \rVert_{\Omega,M}}
        {\lVert D(\mathbf z_\theta)-\mathbf y_0\rVert_{\Omega,M}+\epsilon},\\
    r_{\mathrm{e2e}}
      &=\frac{\lVert D(\mathbf z_\theta)-R_\theta\mathbf y_0
          \rVert_{\Omega,M}}
        {\lVert\mathbf y_0-R_\theta\mathbf y_0\rVert_{\Omega,M}+\epsilon},
        \qquad \epsilon=10^{-8}.
\end{aligned}
\label{eq:razones-transformacion-latente}
\end{equation}

El denominador es, respectivamente, el error de no transformar o de no
canonizar el latente. Por tanto, uno significa que la operación candidata no
reduce ese error y 0,5 que deja como máximo la mitad. Este umbral absoluto se
fijó antes de observar el resultado. Para los ángulos densos, \(R_\theta\) fue
bilineal; para 90, 180 y 270 grados se usó una reindexación exacta de la
cuadrícula. La tercera razón evalúa la composición completa y no solo el
decodificador.

La regularidad local de una órbita se midió con diferencias cíclicas. Para los
vectores aplanados \(\mathbf z_\theta\), sean
\(\Delta\mathbf z_\theta=\mathbf z_{\theta+\delta}-\mathbf z_\theta\) y
\(\Delta^2\mathbf z_\theta=\Delta\mathbf z_{\theta+\delta}
-\Delta\mathbf z_\theta\). La razón normalizada fue

\begin{equation}
\begin{aligned}
    r_{\mathrm{lin}}(\mathbf z)
      &=\frac{\sqrt{\operatorname{media}_{\theta,d}
          (\Delta^2 z_{\theta d})^2}}
        {\sqrt{\operatorname{media}_{\theta,d}
          (\Delta z_{\theta d})^2}+\epsilon},\\
    r_{\mathrm{lin}}(\mathbf x)
      &=\frac{\sqrt{\operatorname{media}_{\theta,p}
          (\Delta^2 x_{\theta p})^2}}
        {\sqrt{\operatorname{media}_{\theta,p}
          (\Delta x_{\theta p})^2}+\epsilon},\\
    \widetilde r_{\mathrm{lin}}(\mathbf z)
      &=\frac{r_{\mathrm{lin}}(\mathbf z)}
        {r_{\mathrm{lin}}(\mathbf x)+\epsilon}.
\end{aligned}
    \label{eq:linealidad-local-corregida}
\end{equation}

Aquí, \(d\) recorre las coordenadas del latente aplanado y \(p\) las de la
imagen RGB aplanada. La división por la misma medida en la órbita RGB controla parte de la
irregularidad introducida por la interpolación de la entrada. Se evaluó con
\(\delta=1^\circ\) y, como control de robustez, con
\(\delta=5^\circ\). Los ángulos son mediciones repetidas de los mismos parches
y no muestras biológicas independientes. Estas medidas constituyen un análisis
funcional descriptivo del conjunto fijo; siguen pendientes pruebas más amplias
de factorización, topología o una acción global aprendida.
...
remuestreando parches como si fueran independientes. Se empleó bootstrap por
conglomerados~\cite{field_bootstrapping_2007}: la WSI fue la unidad de
remuestreo, todos sus parches se transportaron conjuntamente y las mismas
multiplicidades se aplicaron a los dos modelos.

En reconstrucción se generaron \(10\,000\) réplicas con semilla 4501. Cada
réplica seleccionó con reemplazo 23 WSI, sin estratificar, y recalculó la
diferencia entre las medias agrupadas sobre todos los parches transportados;
por ello, una WSI repetida aportó nuevamente todos sus parches. En diagnóstico
se generaron \(10\,000\) réplicas con semilla 3601, remuestreando dentro de
cada uno de los cinco subtipos y conservando el tamaño observado de cada
estrato. En ambos casos se usó el intervalo percentil, cuyos extremos fueron
los cuantiles 0,025 y 0,975 de las diferencias bootstrap.

Para tejido se generaron \(10\,000\) réplicas con semilla 3901. Las 23 WSI se
dividieron según las clases disponibles en 2 láminas con tumor, 16 con tumor y
estroma, y 5 con tumor, estroma y necrosis; el remuestreo se realizó dentro de
esos tres estratos. La familia principal estuvo compuesta por seis
estadísticos: las diferencias de macro-F1 en los cinco presupuestos y la
diferencia del área normalizada bajo la curva de macro-F1 frente a
\(\log_{10}\) del número de etiquetas. Además de los intervalos percentiles
puntuales, se construyó una banda simultánea centrada. Si
\(\widehat\delta_k\) es la diferencia observada para el estadístico \(k\) y
\(\delta^*_{rk}\) la réplica \(r\), se calculó

\begin{equation}
    e_{rk}=\delta^*_{rk}-\widehat\delta_k,\qquad
    c=Q_{0,95}\!\left(\max_k|e_{rk}|\right),\qquad
    I_k^{\mathrm{sim}}=[\widehat\delta_k-c,\widehat\delta_k+c].
    \label{eq:bootstrap-simultaneo-tejido}
\end{equation}

El cuantil \(Q_{0,95}\) se obtuvo con interpolación lineal. Esta banda controla
la lectura conjunta de los seis estadísticos; no debe confundirse con seis
intervalos puntuales independientes.
...
La trazabilidad se organizó alrededor de la identidad de cada WSI. Las
particiones se fijaron por lámina completa, antes de calcular representaciones,
y ninguna WSI apareció en más de un subconjunto. Cada población lógica se
conservó en un manifiesto ordenado por identificador de WSI y coordenada del
parche. Las dos ramas consumieron los mismos índices en el mismo orden, de modo
que toda diferencia se calculó sobre observaciones pareadas y no sobre muestras
reconstruidas de manera independiente.

El código, las configuraciones, los manifiestos, los puntos de control y los
arreglos numéricos usados para tablas y figuras se mantuvieron versionados y
separados de los datos fuente. La selección del punto de control dependió solo
del conjunto de validación. Los presupuestos de etiquetas y sus subconjuntos
anidados también quedaron fijados antes de evaluar la prueba.

Finalmente, la inferencia sobre prueba se ejecutó sin acceso a sus etiquetas.
Primero se guardaron y cerraron las predicciones de ambos modelos; únicamente
después se unieron localmente con la verdad de referencia para calcular las
métricas. No se volvió a entrenar, escoger otro punto de control ni repetir una
configuración a partir del resultado sellado. Estos controles fueron
suficientes para comprobar la separación entre desarrollo y prueba sin hacer
depender la exposición de identificadores internos del repositorio.
...
UBC-OCEAN distribuye el material bajo la licencia CC BY-NC-ND 4.0
~\cite{ubc_ocean_2023}. No se intentó reidentificar a los participantes y el
uso se restringió a investigación no comercial. Las figuras que contienen
material del conjunto identifican su procedencia y las transformaciones
realizadas. Antes del depósito final deberá confirmarse con la instancia
institucional correspondiente si las composiciones derivadas pueden
redistribuirse bajo esa licencia o si deben sustituirse por esquemas propios o
material autorizado. Los resultados se presentan como evidencia
